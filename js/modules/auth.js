...
export function slugify(name) {
  return name.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "invitado";
}
  ...
  script.onload = () => {
    window.google.accounts.id.initialize({
      ...
      callback: (response) => {
        ...
        const profile = { id: "google:" + data.sub, name: data.name, picture: data.picture, provider: "google" };
        setProfile(profile);
        onSignedIn(profile);
      }
    });
    window.google.accounts.id.renderButton(container, { theme: "outline", size: "large", shape: "pill", width: 240 });
  };
...
export function signInAsGuest(name) {
  ...
  const profile = { id: "guest:" + slugify(clean), name: clean, picture: null, provider: "guest" };
  setProfile(profile);
  return profile;
}
